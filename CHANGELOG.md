...
### Added
- 外部打开文件支持**单文件模式**（light）：工作区外的文件不再把所在目录当成项目整窗打开，而是在新窗口以无工作区方式展示（可多文件），编辑器 / 类型服务 / 格式化 / 图片预览按文件所在目录取读写作用域；标题栏、状态栏显示当前文件名，资源管理器提示单文件模式并可一键切到项目模式。工作区内的文件仍留在原窗口打开。
- 应用退出守卫：light 窗口存在未保存的独立文件时先落盘（自动保存开启）或弹确认（自动保存关闭），避免 ⌘Q 静默丢改；关闭单个窗口同样先确认。

### Changed
- 应用重启不再重建退出时的多窗口布局，只打开一个主窗口并恢复主窗口自己的工作区（无记录时回退最近项目）；「在新窗口打开」等运行时的多窗口能力不受影响。
- 画布主视图切换不再交叉淡化，改为整块直接替换，避免离场视图透过入场视图显形。

### Fixed
- 编辑器失焦后快速点击可能误选跨区域文本或跳回旧光标附近：在鼠标按下阶段同步聚焦并恢复滚动位置，移除点击后延迟清选区的竞态；保留双击选词、三击选行、拖选、Shift 扩选及多光标行为。
- 在资源树面板之外选中文本时，⌘C/⌘X 不再被资源树接管，复制与剪切交还系统原生处理，粘贴行为不变。
## [3.3.0] - 2026-09-16
